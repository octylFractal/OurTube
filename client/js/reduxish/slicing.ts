...
export type SliceMap = Map<string, SliceFunction<any, any>[]>;
...
export function createSliceDistributor<STATE extends StringIndexable<any>>(sliceMap: SliceMap, defaultState: STATE): Reducer<STATE, OTAction<any>> {
    // rewrite sliceMap
    // map of actionType -> Slice
    const actionMap: Map<string, Slice<any>> = new Map();
    sliceMap.forEach((v, k) => {
        v.forEach(sliceFunc => {
